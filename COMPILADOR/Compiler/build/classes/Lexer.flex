...
/* Número */
[+|-]?{Numero} { return token(yytext(), "Numero entero", yyline, yycolumn);}
[+|-]?{Numero}\.{Numero} { return token(yytext(), "Numero Flotante", yyline, yycolumn);}

/*DECLARACION Y DEFINICION*/
const { return token(yytext(), "TIPO DE DATO (C)", yyline, yycolumn); }
var { return token(yytext(), "TIPO DE DATO (V)", yyline, yycolumn); }
...
uint |
string |
bool { return token(yytext(), "TIPO DE DATO", yyline, yycolumn);}
...
":=" { return token(yytext(), "Asignacion corta", yyline, yycolumn); }
